...
% Run function
runModelParameterEstimation('2014014', 'C4_04', '2d');


%% Define function to solve muscle redundancy problem with parameter estimation

function runModelParameterEstimation(participant_id, trial_name, model_variant)
    ...
    home_dir = pwd;
    data_path = fullfile(home_dir, '..', 'data', participant_id, 'mrs', trial_name, model_variant);
    
    % Set file details
    ik_file = {fullfile(data_path,[trial_name,'_ik_',model_variant,'.mot'])};
    id_file = {fullfile(data_path,[trial_name,'_id_',model_variant,'.sto'])};
    % % % model_file = fullfile(data_path,[participant_id,'_model_',model_variant,'.osim']);
    ...
    time = [double(trial_data.getIndependentColumn().get(1)) ...
        double(trial_data.getIndependentColumn().get(trial_size-1))];
    ...
    % Set name for output
    Misc.OutName = strjoin({participant_id trial_name model_variant},'_');
